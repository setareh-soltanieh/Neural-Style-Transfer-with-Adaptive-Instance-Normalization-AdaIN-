...
class AdaIN_net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        if self.training:  # training
            ...
            #   your code here ...
            enc_content = self.encode(content)
            enc_style = self.encode(style)

            # apply the adain
            t = self.adain(enc_content[-1], enc_style[-1])
            t = alpha * t + (1-alpha) * enc_content[-1]

            g_t = self.decoder(t)
            g_t_feat = self.encode(g_t)

            # calculate style and content loss
            loss_c = self.content_loss(g_t_feat[-1], t)
            loss_s = self.style_loss(g_t_feat[0], enc_style[0])

            for i in range(1, 4):
                loss_s += self.style_loss(g_t_feat[i], enc_style[i])
            ...
            # apply the adain
            t = self.adain(enc_content[-1], enc_style[-1])
            styled_content = alpha * t + (1 - alpha) * enc_content[-1]

            return self.decode(styled_content)
